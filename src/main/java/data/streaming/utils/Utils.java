...
public class Utils {
	...
	public static TweetDTO insertInBD(TweetDTO t) throws ParseException {
		...
		//Document docu = new Document().append("creationDate", t.getCreatedAt()).append("language", t.getLanguage()).append("text", t.getText()).append("user", t.getUser());
		org.bson.Document userData = new org.bson.Document().append("idStr", t.getUser().getIdStr()).append("name", t.getUser().getName()).append("screenName", t.getUser().getScreenName()).append("friends", t.getUser().getFriends()).append("followers", t.getUser().getFollowers());
		org.bson.Document tweet = new org.bson.Document().append("creationDateLong", dateS.getTime()).append("creationDate", t.getCreatedAt()).append("language", t.getLanguage()).append("text", t.getText()).append("userData", userData);
		
		
		batch.insertOne(tweet);
		
		/*tweetList.add(tweet);
		if(tweetList.size() == 500) {
			batch.insertMany(tweetList);
			tweetList.clear();
		}*/
