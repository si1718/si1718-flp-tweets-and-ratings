	...
	public static void main(String... args) throws Exception {
		...
		*/
		...
		for(int i = 0 ; i < grantDocuments.size() ; i++) {
			...
			System.out.println("Numero de ratings introducidos hasta ahora: " + count);
			System.out.println("Siguiente grant: numero " + i + ", con id: " + grant1.get("idGrant"));
			System.out.println("-----------------------------------------------------");
			for(int j = 1+i ; j < grantDocuments.size() ; j++) {
				...
				if(keywords1.size() > 0 && keywords2.size() >0) {
					...
					if(rating != 0) {
						...
						/*
						// Goal: query sentence: (idGrantA = idGrant1 && idGrantB = idGrant2) || (idGrantA = idGrant2 && idGrantB = idGrant1)
						BasicDBObject orQuery = new BasicDBObject();
						List<BasicDBObject> orQuerySegments = new ArrayList<>();
						
						// First "and" (idGrantA = idGrant1 && idGrantB = idGrant2)
						BasicDBObject andQuery1 = new BasicDBObject();
						List<BasicDBObject> andQuerySegments1 = new ArrayList<>();
						andQuerySegments1.add(new BasicDBObject("idGrantA", grant1.get("idGrant")));
						andQuerySegments1.add(new BasicDBObject("idGrantB", grant2.get("idGrant")));
						andQuery1.put("$and", andQuerySegments1);
						
						orQuerySegments.add(andQuery1);
						
						// Second "and" (idGrantA = idGrant2 && idGrantB = idGrant1)
						BasicDBObject andQuery2 = new BasicDBObject();
						List<BasicDBObject> andQuerySegments2 = new ArrayList<>();
						andQuerySegments2.add(new BasicDBObject("idGrantA", grant2.get("idGrant")));
						andQuerySegments2.add(new BasicDBObject("idGrantB", grant1.get("idGrant")));
						andQuery2.put("$and", andQuerySegments2);
						...
						//System.out.println("Va por el rating (" + grant1.get("idGrant") + "," + grant2.get("idGrant") + ")");
		...
		System.out.println("Finalizado. El total de inserts fue: " + count);
